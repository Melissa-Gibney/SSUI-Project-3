...
export class EventSpec {
    ...
    public bindRegion(regionList : readonly Region[]) : void {
        ...
        // **** YOUR CODE HERE ****
        //Go through each region in the region list
        for(let i = 0; i < regionList.length; i++)
        {
            //Check if the region name matches the given region name
            //If it does, assign that region as the event's region and return
            if(this.regionName === regionList[i].name)
            {
