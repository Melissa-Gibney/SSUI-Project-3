...
export class Transition {
    ...
    public bindTarget(stateList : readonly State[]) : void {
        ...
        //Go through each state in the state list
        for(let i = 0; i < stateList.length; i++)
        {
            //Check if the state name matches the target name
            //If it does, assign that state as the target and return
            if(this.targetName === stateList[i].name)
            {
