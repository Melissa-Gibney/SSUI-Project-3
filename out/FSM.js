...
export class FSM {
    ...
    _finalize() {
        ...
        this.states.forEach(state => {
            state.transitions.forEach(transition => {
                transition.bindTarget(thisRef.states);
                transition.onEvent.bindRegion(thisRef.regions);
                transition.actions.forEach(action => {
                    action.bindRegion(thisRef.regions);
                });
            });
        });
        // need to link all regions back to this object as their parent
        // **** YOUR CODE HERE ****
        //Iterate through the regions and set this as their parent
        this.regions.forEach(region => {
            region.parent = thisRef;
        });
    }
    //. . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . . .
    ...
    actOnEvent(evtType, reg) {
        ...
        if (!this.currentState)
            return;
        // **** YOUR CODE HERE ****
        //Search through each transition until you find the first event that matches
        const thisRef = this;
        this.currentState.transitions.forEach(transition => {
            //console.log(evtType, reg?.name ?? "none");
            if (transition.match(evtType, reg)) {
                //Execute each action of that transition
                transition.actions.forEach(action => {
                    action.execute(evtType, reg);
                });
                thisRef._currentState = transition.target;
                return;
            }
        });
    }
