...
export class Action {
    ...
    execute(evtType, evtReg) {
        ...
        switch (this.actType) {
            case actionTypeStrings[0]:
                {
                    //Set the region's image
                    if (this.onRegion !== undefined) {
                        this.onRegion.imageLoc = this.param;
                    }
                    break;
                }
            case actionTypeStrings[1]:
                {
                    //Clear the region's image
                    if (this.onRegion !== undefined) {
                        this.onRegion.imageLoc = "";
                    }
                    break;
                }
            case actionTypeStrings[3]:
                {
                    //Print the parameter
            ...
            default:
                {
                    //The action type does not match the possible options
                    console.log("Unknown action type: ", this.actType);
                    break;
                }
        }
    }
    ...
    bindRegion(regionList) {
        // **** YOUR CODE HERE ****
        //Go through each region in the FSM until you find this region's name
        for (let i = 0; i < regionList.length; i++) {
            //Check if the region name matches the on region name
            //If it does, assign that region as the on region and return
            if (this.onRegionName === regionList[i].name) {
